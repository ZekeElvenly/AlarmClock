...
def winAlarm():
    ...
    inHrs = Spinbox(winAlarm, textvariable=hrs,from_=0,to=23, width=3, font = ('arial', 14, 'bold'))
    inHrs.grid(row=2, column=1)
    inMin = Spinbox(winAlarm, textvariable=mins, from_=0, to=59, width=3, font = ('arial', 14, 'bold'))
    inMin.grid(row=2, column=2)
    inSec = Spinbox(winAlarm, textvariable=secs, from_=0, to=59, width=3, font = ('arial', 14, 'bold'))
    inSec.grid(row=2, column=3)
    Button(winAlarm, text="Set Alarm", command=lambda : setalarm(inHrs, inMin ,inSec), bg="DodgerBlue2", fg="white", font = ('arial', 14)).grid(row=3, column=2)
